...
while (stock_ts>0).any():
    ...
    # Observed demand
    observedx = f0 * (newprice / price0)**gammastar + np.reshape(np.random.multivariate_normal(noisemean, noisecov, 1), (numvars,1))
...
while (stock_constant>0).any():
    ...
    # Observed demand
    observedx = f0 + np.reshape(np.random.multivariate_normal(noisemean, noisecov, 1), (numvars,1))
    for k in range(len(observedx)):
        if observedx[k,0] < 0:
            observedx[k,0] = 0
        if truth[k,0] == True:
...
plt.title('TS vs Constant pricing with stationary demand function with inventory constraint',fontsize=20)
...
# need to set those that are greater than 50 to be 50 otherwise costs will become negative.
# this is because for some products, in the last iteration they may only have 5 left in stock
# but the demand is more than 5. but we take the observed demand anyway in order to simplify things.
